<div>
  <div class="w-full p-6 mx-auto">
    <div class="{{ !$pendingOrders->isEmpty() ? 'flex flex-wrap -mx-3' : '' }} ">
      @forelse ($pendingOrders as $key => $order)
      ...
      @empty
        <div class="min-h-[75dvh] flex flex-col items-center justify-center gap-4">
          ...
          <a href="{{ route('our-program') }}">
            <x-button
              class="bg-blue-600 hover:bg-blue-700 px-6 mt-4 rounded-full py-6">{{ __('Daftar Kursus') }}</x-button>
          </a>
        </div>
      @endforelse
    </div>
