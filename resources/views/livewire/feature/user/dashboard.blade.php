          ...
          <div class="flex flex-col justify-center w-full h-full">
            ...
            <div class="mt-4 space-y-2">
              ...
              <p class="flex items-center gap-2 text-size-sm">
                <x-lucide-mail class="size-4 font-bold" />{{ $user->email }}
              </p>
              <p class="flex items-center gap-2 text-size-sm">
            ...
            <x-button wire:click="redirectToProfile"
              class="bg-blue-600 text-sm font-bold hover:bg-blue-700 mt-4 rounded-full py-4 w-fit">
              {{ __('Update Profile') }}
            </x-button>
          </div>
          ...
          @if ($students->isEmpty() || $programs->isEmpty())
            <div class="flex flex-col items-center justify-center gap-4">
              ...
              <x-button wire:click="redirectToProgram"
                class="bg-blue-600 font-bold hover:bg-blue-700 w-fit mt-4 rounded-full py-4">
                {{ __('Daftar Kursus') }}
              </x-button>
            </div>
          @else
            <div class="flex flex-col h-full">
              ...
              <div class="mt-2 space-y-2">
                @foreach ($programs->take(2) as $program)
                  <p class="flex items-center gap-2 text-size-sm">
                    <x-lucide-album class="size-4 font-bold" />{{ $program['name'] }} ({{ $program['count'] }}
                    terdaftar)
                  </p>
                @endforeach
                @if ($programs->count() >= 2)
                          ...
                          <div
                            ...
                            <div class="pl-10 pt-1.4 lg:max-w-120 relative -top-1.5 w-auto space-y-3">
                              <h6 class="mb-0 font-semibold leading-normal text-size-sm text-slate-700">Nama Program:
                                {{ $program['name'] }}</h6>
                              <p class="mb-0 font-semibold leading-normal text-size-sm text-slate-700">Jumlah Pendaftar:
                                {{ $program['count'] }} terdaftar</p>
                            </div>
                          </div>
                        @endforeach
                      </x-dialog.header>
              ...
              <x-button wire:click="redirectToProgram"
                class="bg-blue-600 font-bold hover:bg-blue-700 mt-4 rounded-full py-4 w-fit">
                {{ __('Lihat Program') }}
              </x-button>
          ...
          @else
            <div class="flex flex-col h-full">
                ...
                @foreach ($students->take(2) as $student)
                  <p class="flex items-center gap-2 text-size-sm">
                    <x-lucide-graduation-cap
                      class="size-4 font-bold" />{{ Str::limit($student->student_name, 15, '...') }}
                          ...
                          <div
                            ...
                            <div class="pl-10 pt-1.4 lg:max-w-120 relative -top-1.5 w-auto space-y-3">
                              ...
                              <p class="mb-0 font-semibold leading-normal text-size-sm text-slate-700">Program
                                Terdaftar: {{ $student->program->name }}</p>
              ...
              <x-button class="bg-blue-600 hover:bg-blue-700 w-fit mt-4 font-bold rounded-full py-4"
                wire:click="redirectToGrade">
                {{ __('Lihat Nilai!') }}
              </x-button>
              <x-badge class="absolute rounded-full top-8 right-8 bg-green-600 hover:bg-green-500">Siswa Aktif</x-badge>
            </div>
          @endif
        </div>
      </div>
        ...
        @if ($schedules->isEmpty())
          <div class="mb-4 flex gap-4">
            <input type="text" placeholder="Cari nama siswa atau program..."
              ...
              id="searchInput" />
            <x-button type="button" class="flex gap-2 bg-blue-600 hover:bg-blue-700 rounded-full" id="searchButton"
              wire:click="performSearch">
              <x-lucide-search class="size-4" />Search
            </x-button>
          </div>
          ...
          <div class="border-black/12.5 mb-0 rounded-t-2xl border-b-0 border-solid bg-white p-6 pb-0">
            <div class="flex flex-wrap mt-0 -mx-3">
              <div class="flex-none w-7/12 max-w-full px-3 mt-0 lg:w-1/2 lg:flex-none">
                <h6>Jadwal Kursus</h6>
                <p class="mb-0 leading-normal text-size-sm"><span class="ml-1 font-semibold">Intensive English Course
                    (IEC) Denpasar</span></p>
              </div>
            </div>
            <div class="flex-auto p-6 px-0 pb-2">
              <div class="mb-4 flex gap-4">
                <input type="text" placeholder="Cari nama siswa atau program..."
                  class="border rounded-full px-4 py-2 w-full text-sm" wire:model.debounce.1000ms="search"
                  id="searchInput" />
                <x-button type="button" class="flex gap-2 bg-blue-600 hover:bg-blue-700 rounded-full"
                  id="searchButton" wire:click="performSearch">
                  <x-lucide-search class="size-4" />Search
                </x-button>
              </div>
              <div class="overflow-x-auto">
                <table class="items-center w-full mb-0 align-top border-gray-200 text-slate-500">
                  <thead class="align-bottom">
                    <tr>
                      @foreach (['Student Name', 'Program', 'Time Class', 'Daily Class', 'Action'] as $header)
                        <th
                          class="px-6 py-3 font-bold tracking-normal {{ $loop->first ? 'text-left' : 'text-center' }} uppercase align-middle bg-transparent border-b letter border-b-solid text-size-xxs whitespace-nowrap border-b-gray-200 text-slate-400 opacity-70">
                          {{ $header }}
                        </th>
                      @endforeach
                    </tr>
                  ...
                  <tbody>
                      ...
                      <tr>
                        <td class="p-2 align-middle bg-transparent border-b whitespace-nowrap">
                          <div class="flex flex-col justify-center">
                            <h6 class="mb-0 leading-normal text-size-sm">{{ $schedule->registration->student_name }}
                            </h6>
                          </div>
                        </td>
                        <td
                    ...
                    @empty
                      <tr>
                        <td colspan="5" class="text-center py-4 text-gray-500">Tidak ada jadwal ditemukan.</td>
                      </tr>
                    @endforelse
                  </tbody>
                </table>
    ...
    <div class="w-full max-w-full px-3 md:flex-none lg:w-1/3 lg:flex-none">
      @if (!$orders->isEmpty())
        <div
          ...
          <div class="border-black/12.5 mb-4 rounded-t-2xl border-b-0 border-solid bg-white p-6 pb-0">
            <h6>Transaksi</h6>
            <p class="mb-0 leading-normal text-size-sm"><span class="ml-1 font-semibold">Preview order kursus IEC
                Denpasar</span></p>
          </div>
              ...
              <div
                ...
                @foreach ($orders->take(1) as $order)
                  ...
                  <div class="ml-11.252 pt-1.4 lg:max-w-120 relative -top-1.5 w-auto space-y-3">
                    ...
                    <p class="mb-0 font-semibold leading-normal text-size-sm text-slate-700">Harga: Rp
                          ...
                          <div
                            ...
                            <div class="pl-10 pt-1.4 lg:max-w-120 relative -top-1.5 w-auto space-y-3">
                              ...
                              <p class="mb-0 font-semibold leading-normal text-size-sm text-slate-700">Status
                                Pembayaran: {{ $order->payment_status }}</p>
                              <p class="mb-0 font-semibold leading-normal text-size-sm text-slate-700">Waktu Pemesanan:
                                {{ $order->created_at->translatedFormat('l, d F Y H:i:s') }}</p>
                            </div>
                          </div>
                ...
                @endif
                <x-button class="bg-blue-600 hover:bg-blue-700 w-fit mt-2.5 ml-4 font-bold rounded-full py-4"
                  wire:click="redirectToBill">
                  {{ __('Lihat Tagihan') }}
                </x-button>
              </div>
            </div>
      ...
      @else
        <div
          ...
          <div class="flex flex-col items-center justify-center gap-4">
            ...
            <x-button wire:click="redirectToProgram"
              class="bg-blue-600 hover:bg-blue-700 w-fit mt-4 font-bold rounded-full py-4">
